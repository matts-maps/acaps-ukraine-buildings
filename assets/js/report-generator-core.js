/* ============================================================================
   ...
   per-page config file (report-generator.js / oblast-report-generator.js)
   that calls window.EPACCReportGenerator.init({...}) with the handful of
   labels/keys that differ between the two views.
...
(function () {
  ...
  // --------------------------------------------------------------------
  function getReportState(config) {
    const state = window.__mapReportState;
    const yearEl = document.getElementById(BASE_IDS.yearSelect);
    const aggEl = document.getElementById(BASE_IDS.aggSelect);
    const startEl = document.getElementById(BASE_IDS.startSelect);
    const endEl = document.getElementById(BASE_IDS.endSelect);
    const totalEl = document.getElementById(BASE_IDS.totalValue);
    const filterGroup = document.getElementById(BASE_IDS.activeFilterGroup);
    const filterLabel = document.getElementById(BASE_IDS.activeFilterLabel);
    ...
    if (state) {
      return Object.assign({
        ...
        activeFilterText,
        [config.countsKey]: state[config.countsKey] || {},
  ...
  async function generateReport(config) {
    ...
    try {
      ...
      const state = getReportState(config);
      ...
      doc.text(config.reportTitleLine, margin, y);
      y += 25;
      doc.setFontSize(9);
      ...
      y += 12;
      const topDimension = topEntry(state[config.countsKey]);
      ...
      doc.setFont("helvetica", "normal");
      doc.setFontSize(9.5);
      const leftRaw = config.buildLeftStats(state);
      const rightRaw = [
        topDimension ? `Most affected: ${topDimension[0]} (${topDimension[1].toLocaleString()})` : "Most affected: N/A",
      ...
      y += statBoxHeight + 25;
      const mapEl = document.getElementById(BASE_IDS.mapContainer);
      const mapImg = await captureMap(mapEl);
      if (mapImg) {
        y = addImageWithHeading(doc, config.mapHeading, mapImg, y, margin, pageWidth, pageHeight, pageWidth - margin * 2);
      } else {
      ...
      if (series.timeline && series.timeline.labels.length) {
        ...
        y = await addSvgWithHeading(doc, "Timeline of damaged buildings", timelineSvg, y, margin, pageWidth, pageHeight, timelineWidth, margin, timelineHeight, "map-timeline-chart");
      }
      const gridGap = 16;
      ...
      let rowYStart = y;
      let maxRowHeight = 0;
      const topSeries = series[config.seriesKey];
      if (topSeries && topSeries.labels.length) {
        const topSvg = buildHorizontalBarSVG(topSeries.labels, topSeries.values, colChartWidth, SUMMARY_CHART_HEIGHT_PX, highlightSetFor(config.dimension, state.activeFilter));
        const nextY = await addSvgWithHeading(doc, config.topChartLabel, topSvg, rowYStart, margin, pageWidth, pageHeight, colChartWidth, margin, SUMMARY_CHART_HEIGHT_PX, "map-top-oblasts-chart");
      ...
      if (series.infra && series.infra.labels.length) {
        ...
        const nextY = await addSvgWithHeading(doc, "Damage by infrastructure type", infraSvg, rowYStart, margin, pageWidth, pageHeight, colChartWidth, margin + colChartWidth + gridGap, SUMMARY_CHART_HEIGHT_PX, "map-infra-type-chart");
        maxRowHeight = Math.max(maxRowHeight, nextY - rowYStart);
      }
      ...
      if (series.extent && series.extent.labels.length) {
        ...
        y = await addSvgWithHeading(doc, "Level of damage", extentSvg, y, margin, pageWidth, pageHeight, colChartWidth, centerX, SUMMARY_CHART_HEIGHT_PX, "map-extent-chart");
      }
      ...
      doc.save(`EPACC_${config.filenamePrefix}_Report_${safeYear}.pdf`);
    } catch (err) {
  ...
  function injectButton(config) {
    ...
    btn.addEventListener("click", () => generateReport(config));
  ...
  function init(config) {
    function start() {
      ...
      if (!document.getElementById("generate-report-btn")) {
        console.warn(`${config.filenamePrefix.toLowerCase()}-report-generator: could not find '.map-hint' to attach the button near.`);
      }
    }
