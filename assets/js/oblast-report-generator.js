/* ============================================================================
   E-PACC UKRAINE - "Generate PDF Report" for oblast_analysis.html
   ============================================================================

   INSTALL
   -------
   1. Add the hook in oblast_analysis.js so window.__mapReportState is
      populated with the real numbers behind the current view (see
      processMapVisualisations / updateSummaryCharts).

   2. Add these CDN libraries to oblast_analysis.html, then this file, all
      AFTER the existing Leaflet / Chart.js / oblast_analysis.js scripts.
...
(function () {
  ...
  const IDS = {
    ...
    charts: {
      ...
      topOblasts: { id: "map-top-oblasts-chart", label: "Top Oblasts by Reported Damage" },
  ...
  function getReportState() {
    ...
    if (state) {
      return {
        ...
        activeFilterText,
        oblastCounts: state.oblastCounts || {},
  ...
  // Horizontal bar chart (Top Oblasts / Infra Type)
  ...
  // ... [Rest of the file remains unchanged: svgToPngDataUrl, embedSvgChart, etc.] ...
  ...
  async function generateReport() {
    ...
    try {
      ...
      doc.text("Oblast Damage Analysis Report", margin, y);
      y += 25;
      doc.setFontSize(9);
      ...
      y += 12;
      const topOblast = topEntry(state.oblastCounts);
      ...
      if (mapImg) {
        y = addImageWithHeading(doc, "Damage Buildings per Oblast", mapImg, y, margin, pageWidth, pageHeight, pageWidth - margin * 2);
      } else {
      ...
      const rowHeadingHeight = Math.max(
        series.topOblasts && series.topOblasts.labels.length ? measureHeadingHeight(doc, IDS.charts.topOblasts.label, colChartWidth) : 0,
      ...
      if (series.topOblasts && series.topOblasts.labels.length) {
        const topOblastsSvg = buildHorizontalBarSVG(series.topOblasts.labels, series.topOblasts.values, colChartWidth, SUMMARY_CHART_HEIGHT_PX, highlightSetFor("oblast", state.activeFilter));
        const nextY = await addSvgWithHeading(doc, IDS.charts.topOblasts.label, topOblastsSvg, rowYStart, margin, pageWidth, pageHeight, colChartWidth, margin, SUMMARY_CHART_HEIGHT_PX, IDS.charts.topOblasts.id);
      ...
      doc.save(`EPACC_Oblast_Report_${safeYear}.pdf`);
    } catch (err) {
  ...
  function init() {
    ...
    if (!document.getElementById("generate-report-btn")) {
      console.warn("report-generator.js: could not find '.map-hint' to attach the button near.");
    }
  }
